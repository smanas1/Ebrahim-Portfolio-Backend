...
const createProduct = async (req, res) => {
    try {
        const { category, brandName, productName, productDetails, moq, costOfGoods, sampleCost, shipToUsa, asin, } = req.body;
...
const updateProduct = async (req, res) => {
    try {
        const { id } = req.params;
        const { category, brandName, productName, productDetails, moq, costOfGoods, sampleCost, shipToUsa, asin, existingPictures, // This will hold the existing pictures to keep
         } = req.body;
        const files = req.files;
        const product = await Product_1.Product.findById(id);
        if (!product) {
